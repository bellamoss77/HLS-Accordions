...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        // Check for open sub-accordions first
        const openSubItem = document.querySelector(".item.active .sub-item.active");
        if (openSubItem) {
            const subHeader = openSubItem.querySelector('.sub-title');
            subHeader.click(); // Simulate a click on the sub-accordion's header to close it
            return;
        }

        // If no sub-accordion is open, then check for an open main accordion
        if (currentItem !== null) {
            const mainHeader = items[currentItem].querySelector(".section-header");
            mainHeader.click(); // Simulate a click on the main accordion's header to close it
            currentItem = null;
        }
    }
});
